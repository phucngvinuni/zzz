...
def train(rank, total_num_gpus, argv):
    ...
    if FLAGS.restart_dir is not None:
        ...
        num_epochs = math.ceil(FLAGS.total_steps / steps_per_epoch - global_step / steps_per_epoch)
    else:
        global_step = 0
        ...
        for epoch in epoch_pbar:
            ...
            with trange(steps_per_epoch, dynamic_ncols=True) as step_pbar:
                for step in step_pbar:
                    ...
                    if (FLAGS.save_step > 0 and global_step % FLAGS.save_step == 0) or global_step == 1: #or global_step == checkpoint["step"]+1:
