...
import sys
sys.path.append('./torchcfm/models/unet')
...
class EncoderVAE(nn.Module):
    ...
    def encode(self, x):
        ...
        # mean, logvar = th.chunk(out, 2, dim=1)
        # std = th.exp(0.5 * logvar)
        # sample = mean + std * th.randn(mean.shape).to(device=x.device)
        return out
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        if self.num_latents is not None:
            ...
            self.latent_mlp = nn.Linear(math.floor(4/2), time_embed_dim)
            ...
            self.latent_encodings = nn.Linear(self.num_latents, 4)
